...
function generateMetaTags(lang, translations, pageName) {
  ...
  // Add Open Graph URL
  const pagePath = pageName === 'index.html' ? '' : pageName.replace('.html', '');
  ...
  if (pageName === 'index.html') {
    metaTags.push(`<link rel="amphtml" href="${baseUrl}/${lang}/amp">`);
  }
      ...
      if (componentPath.endsWith('.json')) {
        ...
        if (isCriticalComponent) {
          ...
          if (componentName === 'NAVBAR') {
            ...
            const langPathEn = isIndexPage ? '../en/' : '../en/' + currentPage.replace('.html', '');
            const langPathEt = isIndexPage ? '../et/' : '../et/' + currentPage.replace('.html', '');
            const langPathRu = isIndexPage ? '../ru/' : '../ru/' + currentPage.replace('.html', '');
...
async function generateSitemap() {
  ...
  try {
    // Use consistent ISO8601 format without milliseconds for all sitemaps
    const now = new Date();
    const todayISO = now.toISOString().split('.')[0] + '+03:00';
...
`;
      ...
      for (const template of templates) {
        ...
        langSitemapContent += `    <lastmod>${todayISO}</lastmod>\n`;
...
`;
    ...
    for (const lang of languages) {
      ...
      ampSitemapContent += `    <lastmod>${todayISO}</lastmod>\n`;
